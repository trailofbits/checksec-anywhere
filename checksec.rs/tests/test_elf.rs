use checksec::{binary::BinSpecificProperties, checksec, elf, shared};
mod utils;
...
fn test_clang_safestack_exists() {
    ...
    match &checksec(&buf, filename).blobs[0].properties {
        BinSpecificProperties::Elf(elf_result) => {
            assert!(elf_result.clang_safestack)
        }
        _ => {
            panic!("Checksec failed")
        }
...
fn test_get_architecture() {
    ...
    match &checksec(&buf, filename).blobs[0].properties {
        BinSpecificProperties::Elf(elf_result) => {
            assert_eq!(elf_result.architecture, "X86_64".to_string())
        }
        _ => {
            panic!("Checksec failed")
        }
...
fn test_seperate_code_on() {
    ...
    match &checksec(&buf, filename).blobs[0].properties {
        BinSpecificProperties::Elf(elf_result) => {
            assert!(elf_result.seperate_code)
        }
        _ => {
            panic!("Checksec failed")
        }
