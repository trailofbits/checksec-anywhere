use checksec::{binary::BinSpecificProperties, checksec, shared};
mod utils;
...
fn test_is_macho() {
    ...
    match &checksec(&buf, filename).blobs[0].properties {
        ...
        _ => {
            panic!("Expected Binary to be classified as Mach-O");
        }
    }
}
...
fn test_has_codesig() {
    ...
    match &checksec(&buf, filename).blobs[0].properties {
        BinSpecificProperties::MachO(macho_result) => {
            assert!(macho_result.code_signature)
        }
        _ => {
            panic!("Checksec failed")
        }
...
fn test_rpath() {
    ...
    match &checksec(&buf, filename).blobs[0].properties {
        BinSpecificProperties::MachO(macho_result) => {
            let runpath_vec = shared::VecRpath::new(vec![
                shared::Rpath::Yes("@executable_path/lib".into()),
                shared::Rpath::Yes("./src".into()),
            ]);
            assert_eq!(macho_result.rpath.len(), runpath_vec.len());
            assert_eq!(
                macho_result.rpath[0],
                shared::Rpath::Yes("@executable_path/lib".into())
            );
            assert_eq!(
                macho_result.rpath[1],
                shared::Rpath::Yes("./src".into())
            );
        }
        _ => {
...
fn test_architecture_id() {
    ...
    match &checksec(&buf, filename).blobs[0].properties {
        BinSpecificProperties::MachO(macho_result) => {
            assert_eq!(macho_result.architecture, "x86_64".to_string())
        }
        _ => {
            panic!("Checksec failed")
        }
    ...
    match &checksec(&buf, filename).blobs[0].properties {
        BinSpecificProperties::MachO(macho_result) => {
            assert_eq!(macho_result.architecture, "arm64".to_string())
        }
        _ => {
            panic!("Checksec failed")
        }
...
fn test_symbol_count() {
    ...
    match &checksec(&buf, filename).blobs[0].properties {
        BinSpecificProperties::MachO(macho_result) => {
            assert_eq!(macho_result.symbol_count, 5)
        }
        _ => {
            panic!("Checksec failed")
        }
...
fn test_dynamic_linking() {
    ...
    match &checksec(&buf, filename).blobs[0].properties {
        BinSpecificProperties::MachO(macho_result) => {
            assert!(macho_result.dyn_linking)
        }
        _ => {
            panic!("Checksec failed")
        }
