use checksec::{checksec, pe, binary::BinSpecificProperties, compression::{compress, decompress}};
mod utils;
...
fn test_roundtrip() {
    ...
    match &checksec(&buf, filename).blobs[0].properties {
        BinSpecificProperties::PE(result) => {
            ...
            let decompress_result: pe::CheckSecResults = decompress(compressed.as_bytes()).expect("decompress_results failed");
